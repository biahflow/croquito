UV_CACHE_DIR ?= /private/tmp/uv-cache-croquitodxf
XDG_CACHE_HOME ?= /private/tmp/croquitodxf-xdg-cache
MPLCONFIGDIR ?= /private/tmp/croquitodxf-matplotlib-cache
export UV_CACHE_DIR
...
db-init:
	set -a; test ! -f .env.local || . ./.env.local; set +a; uv run python -m croquitodxf_api.bootstrap

dev-api:
	set -a; test ! -f .env.local || . ./.env.local; set +a; uv run uvicorn croquitodxf_api.main:app --reload --port 8000

dev-web:
	npm run web:dev

# UI local de homologação da medição (porta 5174). Ela fala com o servidor local
# `croquitodxf-valuation serve` (porta 8801), não com a API do croqui, por isso fica
# fora do `make dev`.
dev-medicao:
	npm run medicao:dev

dev-worker:
	set -a; test ! -f .env.local || . ./.env.local; set +a; uv run croquitodxf-demo local-worker-once
...
dev-worker-fixtures:
	set -a; test ! -f .env.local || . ./.env.local; set +a; uv run croquitodxf-demo local-worker-once --fixtures

contracts:
	uv run python -m croquitodxf_core.schema_export --output packages/contracts/scene.schema.json
...
check:
	...
	uv run python -m croquitodxf_core.schema_export --check packages/contracts/scene.schema.json
...
demo:
	uv run croquitodxf-demo synthetic --output output/demo

provider-contract-demo:
	uv run croquitodxf-demo provider-contract-demo --output output/provider-contract-demo

vision-eval:
	uv run croquitodxf-demo vision-eval --output output/vision-eval

solver-eval:
	uv run croquitodxf-demo solver-eval --output output/solver-eval

valuation-demo:
	uv run croquitodxf-valuation demo --output output/valuation-demo

valuation-eval:
	uv run croquitodxf-valuation takeoff-eval --output output/valuation-eval
...
valuation-extraction-eval:
	uv run croquitodxf-valuation extraction-eval --output output/valuation-extraction-eval
...
valuation-parity:
	...
	uv run croquitodxf-valuation parity --previous "$(PREVIOUS)" --output output/valuation-parity
...
valuation-compare:
	...
	uv run croquitodxf-valuation compare-bulletin --valuation "$(VALUATION)" --worksite "$(WORKSITE)" \
...
# documento em CROQUITODXF_AI_EXTRACTION_ALLOWED_DIGESTS; sem isso recusa antes de sair.
extraction-eval:
	set -a; test ! -f .env.local || . ./.env.local; set +a; uv run croquitodxf-demo extraction-eval \
