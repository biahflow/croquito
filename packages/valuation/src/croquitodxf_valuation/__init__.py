"""Contexto de medição de obra pública: do boletim canônico à planilha auditada."""

from croquitodxf_valuation.assignment import (
    SCO_HYBRID_SUGGESTER_VERSION,
    SCO_REFINED_HYBRID_SUGGESTER_VERSION,
    SCO_REFINED_SUGGESTER_VERSION,
    SCO_SUGGESTER_VERSION,
    CodeAssignment,
    CodeAssignmentBatch,
    CodeAssignmentInput,
    CodeAssignmentSet,
    CodeCandidate,
    CodeSuggestion,
    CodeSuggestionSet,
    SuggestionConfig,
    SuggestionRefinement,
    SuggestionSemantics,
    apply_code_assignments,
    apply_refinement,
    suggest_codes,
)
from croquitodxf_valuation.calc import (
    CalcBlockPlan,
    CalcBuildResult,
    CalcPlan,
    ItemCalcPlan,
    build_worksite_bulletin,
    build_worksite_valuation,
)
from croquitodxf_valuation.canonical import (
    AUDIT_SCHEMA_VERSION,
    AuditedWorksite,
    AuditFinding,
    AuditReport,
    audit_workbook,
    canonicalize_workbook,
)
from croquitodxf_valuation.catalog import (
    StemIdfTable,
    build_stem_idf_table,
    lexical_similarity,
    lexical_tokens,
    normalize_unit,
    query_coverage_score,
    read_price_catalog,
    weighted_query_coverage_score,
)
from croquitodxf_valuation.contract import (
    CONTRACT_SCHEMA_VERSION,
    Amendment,
    AmendmentLine,
    ContractLine,
    ContractWorkbook,
    PeriodProgress,
    contract_workbook_id_for,
)
from croquitodxf_valuation.contract_diagnosis import (
    CONTRACT_SEMANTICS_DIVERGENT,
    ContractDiagnosis,
    ContractParse,
    ContractSemanticsError,
    SemanticFinding,
    diagnose_contract,
)
from croquitodxf_valuation.errors import (
    ValuationValidationError,
    valuation_error_codes,
    valuation_errors,
)
from croquitodxf_valuation.models import (
    BulletinLine,
    CalcBlock,
    CalcOperand,
    CalcRecipe,
    CalcSheet,
    PriceCatalog,
    PriceCatalogEntry,
    ReviewerDecision,
    Valuation,
    ValuationApproval,
    ValuationContractModel,
    WorksiteBulletin,
)
from croquitodxf_valuation.rounding import money_trunc, quantity_round, trunc_divergence
from croquitodxf_valuation.sco import ScoCodeParts, parse_sco_code
from croquitodxf_valuation.takeoff import (
    TAKEOFF_SCHEMA_VERSION,
    PlateBox,
    PlateEvidence,
    TakeoffDecisionBatch,
    TakeoffDecisionInput,
    TakeoffItem,
    TakeoffItemStatus,
    TakeoffPacket,
    apply_takeoff_decisions,
    load_takeoff_packet,
)
from croquitodxf_valuation.template import (
    AmendmentColumns,
    AmendmentLayout,
    GeneralLayout,
    WorkbookTemplate,
    default_template,
)
from croquitodxf_valuation.workbook_reader import (
    MAX_ROW_FAILURES,
    ContractImportNotes,
    NormalizedCell,
    read_contract_parse,
    read_contract_workbook,
    read_contract_workbook_with_notes,
)
from croquitodxf_valuation.workbook_writer import (
    PinnedCell,
    PlannedWorksite,
    WorkbookPlan,
    WriteReport,
    plan_workbook,
    write_valuation_workbook,
)

__all__ = [
    "AUDIT_SCHEMA_VERSION",
    "CONTRACT_SCHEMA_VERSION",
    "CONTRACT_SEMANTICS_DIVERGENT",
    "MAX_ROW_FAILURES",
    "SCO_HYBRID_SUGGESTER_VERSION",
    "SCO_REFINED_HYBRID_SUGGESTER_VERSION",
    "SCO_REFINED_SUGGESTER_VERSION",
    "SCO_SUGGESTER_VERSION",
    "TAKEOFF_SCHEMA_VERSION",
    "Amendment",
    "AmendmentColumns",
    "AmendmentLayout",
    "AmendmentLine",
    "AuditFinding",
    "AuditReport",
    "AuditedWorksite",
    "BulletinLine",
    "CalcBlock",
    "CalcBlockPlan",
    "CalcBuildResult",
    "CalcOperand",
    "CalcPlan",
    "CalcRecipe",
    "CalcSheet",
    "CodeAssignment",
    "CodeAssignmentBatch",
    "CodeAssignmentInput",
    "CodeAssignmentSet",
    "CodeCandidate",
    "CodeSuggestion",
    "CodeSuggestionSet",
    "ContractDiagnosis",
    "ContractImportNotes",
    "ContractLine",
    "ContractParse",
    "ContractSemanticsError",
    "ContractWorkbook",
    "GeneralLayout",
    "ItemCalcPlan",
    "NormalizedCell",
    "PeriodProgress",
    "PinnedCell",
    "PlannedWorksite",
    "PlateBox",
    "PlateEvidence",
    "PriceCatalog",
    "PriceCatalogEntry",
    "ReviewerDecision",
    "ScoCodeParts",
    "SemanticFinding",
    "StemIdfTable",
    "SuggestionConfig",
    "SuggestionRefinement",
    "SuggestionSemantics",
    "TakeoffDecisionBatch",
    "TakeoffDecisionInput",
    "TakeoffItem",
    "TakeoffItemStatus",
    "TakeoffPacket",
    "Valuation",
    "ValuationApproval",
    "ValuationContractModel",
    "ValuationValidationError",
    "WorkbookPlan",
    "WorkbookTemplate",
    "WorksiteBulletin",
    "WriteReport",
    "apply_code_assignments",
    "apply_refinement",
    "apply_takeoff_decisions",
    "audit_workbook",
    "build_stem_idf_table",
    "build_worksite_bulletin",
    "build_worksite_valuation",
    "canonicalize_workbook",
    "contract_workbook_id_for",
    "default_template",
    "diagnose_contract",
    "lexical_similarity",
    "lexical_tokens",
    "load_takeoff_packet",
    "money_trunc",
    "normalize_unit",
    "parse_sco_code",
    "plan_workbook",
    "quantity_round",
    "query_coverage_score",
    "read_contract_parse",
    "read_contract_workbook",
    "read_contract_workbook_with_notes",
    "read_price_catalog",
    "suggest_codes",
    "trunc_divergence",
    "valuation_error_codes",
    "valuation_errors",
    "weighted_query_coverage_score",
    "write_valuation_workbook",
]
