"""Importador offline do catálogo SINAPI: leitor .xlsx mínimo + catálogo canônico.

A tabela SINAPI (Sistema Nacional de Pesquisa de Custos e Índices da Construção Civil,
IBGE/Caixa) é publicada mensalmente e é de distribuição pública, mas o arquivo real não
existe neste repositório — tudo aqui é sintético (`croquito_worker.valuation.sinapi_fixture`),
e o layout real (aba, linha de cabeçalho, colunas, padrão de código) fecha como DADO em
`SinapiCatalogLayout`, nunca como constante de código (ADR-0039, decisão 2 — o mesmo
desenho do importador EMOP, `croquito_valuation.emop`, que é o molde deste módulo).

Regra da orçamentista (M8, `docs/architecture/VALUATION_CONTEXT.md`): em obra LICITADA o
contrato manda — preço nunca vem da SINAPI. Este módulo só importa o catálogo com
`origin=PriceOrigin.SINAPI` (`models.py`); é o guardrail em
...
recusa fechado (`SINAPI_SHEET_MISSING`/`SINAPI_FIELD_MISSING`). Diferente do leitor de
...
(`SINAPI_ROW_UNPARSEABLE`) em vez de converter. Linha inteiramente em branco (as quatro
colunas vazias) é pulada e contada — nunca vira entrada do catálogo, o mesmo tratamento
que o registro deletado tem no EMOP.

## Família e subgrupo

O layout declarado no contrato desta fonte (`SinapiCatalogLayout`) não tem coluna de
...
(`SINAPI_FAMILY_CODE` e `layout.source_label`): a tabela inteira nasce em uma família só.
...
família/subgrupo de um catálogo `sinapi`, então a simplificação não perde informação que
...
SINAPI_FAMILY_CODE: Final = "SINAPI"
"""Família/subgrupo únicos que todo catálogo `sinapi` importado por este módulo recebe —
ver a seção "Família e subgrupo" no docstring do módulo."""

_SINAPI_CATALOG_ID_NAMESPACE: Final = uuid5(
    NAMESPACE_URL, "https://croquito.local/valuation/catalog/sinapi"
)


def sinapi_catalog_id_for(source_sha256: str) -> UUID:
    """Id derivado do conteúdo do .xlsx: reimportar o mesmo arquivo devolve o mesmo catálogo.

    Namespace próprio, distinto do `catalog_id_for` (SCO) e do `emop_catalog_id_for`: as
    fontes nunca deveriam colidir de id mesmo que o SHA-256 dos bytes coincidisse por
    acaso, porque são catálogos de naturezas diferentes.
    """
    return uuid5(_SINAPI_CATALOG_ID_NAMESPACE, source_sha256)


class SinapiCatalogLayout(ValuationContractModel):
    """Layout do catálogo SINAPI como DADO: aba, cabeçalho e colunas do .xlsx.

    Espelha `EmopCatalogLayout` (`emop.py`): nenhuma posição é constante do código.
    `code_pattern` é o padrão REAL do código SINAPI — regex como dado, validada na
    construção com o mesmo cinto e suspensório de `EmopCatalogLayout.validate_code_pattern`
    (compila e não casa a string vazia). Toda linha lida ainda revalida contra o superset
    estrutural de `PriceCatalogEntry.validate_code_for_origin` (`NON_SCO_CODE_PATTERN`): um
    ...
    def validate_code_pattern(self) -> SinapiCatalogLayout:
        try:
            compiled = re.compile(self.code_pattern)
        except re.error as error:
            raise ValuationValidationError(
                "SINAPI_LAYOUT_CODE_PATTERN_INVALID",
                "padrão de código do layout SINAPI não compila como expressão regular",
                {"pattern": self.code_pattern, "reason": str(error)},
            ) from error
        if compiled.fullmatch("") is not None:
            raise ValuationValidationError(
                "SINAPI_LAYOUT_CODE_PATTERN_INVALID",
                "padrão de código do layout SINAPI casa a string vazia; frouxo demais "
...
def _xlsx_unsupported(reason: str, **details: object) -> NoReturn:
    raise ValuationValidationError("SINAPI_XLSX_UNSUPPORTED", reason, details)


def _row_error(row_number: int, reason: str, **details: object) -> NoReturn:
    raise ValuationValidationError(
        "SINAPI_ROW_UNPARSEABLE",
        f"linha ilegível no catálogo SINAPI (SINAPI_ROW_UNPARSEABLE:{row_number}): {reason}",
...
def read_sinapi_catalog_with_report(
    xlsx_path: Path, layout: SinapiCatalogLayout
) -> tuple[PriceCatalog, SinapiImportNotes]:
    """Lê a tabela .xlsx do catálogo SINAPI e devolve o catálogo canônico e as notas da leitura.

    Fail-closed, na ordem: estrutura do .xlsx em si (`SINAPI_XLSX_UNSUPPORTED`), aba
    declarada ausente (`SINAPI_SHEET_MISSING`), coluna declarada fora da extensão real da
    tabela (`SINAPI_FIELD_MISSING`), depois linha a linha — linha inteiramente em branco é
    ...
    violação estrutural do modelo (`SINAPI_ROW_UNPARSEABLE` em todos os casos, com o
    número da linha). Tabela sem nenhuma entrada válida recusa (`SINAPI_EMPTY`); código
    duplicado recusa pelo `CATALOG_DUPLICATE_CODE` que o próprio `PriceCatalog` já garante.
    O catálogo devolvido nasce inteiro com `origin=PriceOrigin.SINAPI` — é esse dado que os
    ...
    try:
        if layout.sheet_name not in workbook.sheetnames:
            raise ValuationValidationError(
                "SINAPI_SHEET_MISSING",
                "planilha SINAPI não possui a aba declarada no layout",
        ...
        if missing:
            raise ValuationValidationError(
                "SINAPI_FIELD_MISSING",
                "coluna declarada no layout SINAPI está fora da extensão real da tabela",
    ...
    catalog = PriceCatalog(
        id=sinapi_catalog_id_for(source_sha256),
    ...
    notes = SinapiImportNotes(blank_row_count=len(blank_rows), blank_rows=tuple(blank_rows))
    return catalog, notes


def read_sinapi_catalog(xlsx_path: Path, layout: SinapiCatalogLayout) -> PriceCatalog:
    """Lê o catálogo SINAPI a partir do .xlsx, descartando as notas da leitura."""
    catalog, _ = read_sinapi_catalog_with_report(xlsx_path, layout)
    return catalog
