...
| VAL-12 | Índice de embeddings publicado no acervo e braço semântico no recompute (`index-catalog` do CLI, tabela `reference_catalog_embeddings`, rotas `/v1/platform/reference-catalog-indexes`, resolução por digest, notas de degradação por fonte) | ADR-0052/0021/0047 | `tests/valuation/test_matcher_golden.py` (gate `recall@20 = 100%` do braço híbrido, exige catálogo real + índice + cache locais); `tests/api/test_reference_catalog_indexes.py` (publicação, quatro recusas e teto próprio de leitura); `tests/api/test_migrations.py`; `tests/api/test_estimate_semantic_arm.py` (leitura do índice na cascata: `GET` sem provider nenhum, degradação declarada por fonte, cobertura parcial, espaços vetoriais divergentes, fiação com `build_hybrid_code_suggestions_over_cascade`); **cadeia de medição licitada ainda não lê o índice** (fatia própria), e o recompute real contra o índice pago em homologação segue pendente (ato do operador) |
