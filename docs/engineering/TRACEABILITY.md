...
| VAL-14 | Desfazer um par `(item_id, code)` confirmado: revogação como decisão nova, registro em `revocations`, reabertura do pacote e compensação do índice de precedentes | ADR-0061/0053/0046 | `tests/valuation/test_assignment.py` (o par sai do conjunto, o fechamento cai, o mesmo par volta a ser decidível, regime `1.0.0` recusa, id determinístico em espaço próprio `vr_`); `tests/api/test_precedents.py` (a observação do par desfeito sai do índice, só a desta praça, e refechar reindexa); `tests/api/test_valuation_round_routes.py` (a rota irmã da medição, sem efeito de índice, e o conflito de versão); `tests/api/test_estimate_round_routes.py` (recusa provisória depois da aprovação — ADR-0061 D7); `apps/web/src/orcamento/revogacao.test.tsx` (motivo obrigatório, efeito escrito antes do clique, botão que muda de nome com o pacote fechado, lista de desfeitos e o par reconfirmado que sai dela) |
