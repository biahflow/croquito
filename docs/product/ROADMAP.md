...
| F-013 | A DEFINIR | READY_FOR_SPEC | [Ver e administrar quem tem acesso, sem abrir o Keycloak](../features/F-013-ui-de-membros-do-tenant/feature.md) — **ganhou Feature Contract em 2026-09-05**. Destravou junto com a F-008 (o convite que ela reusa); precisa da F-014 antes, porque o unknown 2 de lá decide onde o vínculo usuário↔tenant vive |
| F-014 | A DEFINIR | READY_FOR_SPEC | [O tenant existe como entidade, e um cliente novo entra sem SQL](../features/F-014-tenant-e-onboarding/feature.md) — **ganhou Feature Contract em 2026-09-05**, escolhida como base do inventário SaaS: hoje `tenant_id` é string em 36 colunas e atributo no Keycloak, sem tabela de tenants. Um ADR precede o planejamento (o unknown 1: `tenant_id` vira FK ou continua string com a tabela ao lado) |
| F-015 | A DEFINIR | READY_FOR_SPEC | [Reprocessar um PDF já enviado, sem subi-lo de novo](../features/F-015-recriar-job-de-upload-usado/feature.md) — **ganhou Feature Contract em 2026-09-05**, já encolhida pela auditoria de agosto (a allowlist saiu com a F-012). A trava restante é `jobs.upload_id UNIQUE`, que o ADR-0049 mantém de propósito — mexer nela é ADR próprio. Recomendação: `LOW` entre as cinco (o contorno é subir o arquivo de novo) |
| F-016 | A DEFINIR | READY_FOR_SPEC | Rotação de chaves e segredos de provider (a definir em contrato) |
| F-017 | A DEFINIR | READY_FOR_SPEC | [A trilha de autorização de IA vira tela](../features/F-017-trilha-de-auditoria-do-entitlement/feature.md) — **ganhou Feature Contract em 2026-09-05**, com metade já entregue pela F-031 (custo por tenant). O que resta é a trilha — e o desenho já existe: é o **estado 6 reservado** do pacote aprovado da F-034 |
...
| F-048 | HIGH | PLANNING | [Auditoria de medição de terceiros: o furo que ninguém procurou](../features/F-048-auditoria-de-medicao-de-terceiros/feature.md) — contrato em 2026-09-05, **prioridade `HIGH` por ato humano no mesmo dia**: o motor já existe e já achou dinheiro errado em arquivo real (os 8 erros de centavo do MAPÃO do cliente viram o caso de aceite). Faltam rota `/v1`, artefato de relatório e tela; **o unknown 2 foi respondido em 2026-09-05 — produto avulso**, jornada própria que não exige projeto nem contrato (é o que alcança quem ainda não é cliente); **Design Approval Package aprovado em 2026-09-05** (revisão 1) e **plano escrito no mesmo dia** ([plan.md](../features/F-048-auditoria-de-medicao-de-terceiros/plan.md), 7 tarefas em cadeia). O planejamento achou duas lacunas entre contrato e código e as declarou: de que **layout** é a planilha que entra (os motores exigem `WorkbookTemplate` e a API nunca recebeu um — `ARCHITECTURE_DECISION_REQUIRED`) e que o comparador exige um `Valuation` do croquito, inexistente no avulso (vira a T1). Dois gates humanos antes do build |
| F-049 | MEDIUM | READY_FOR_PLANNING | [Dashboard executivo do contrato: planejado, realizado e saldo](../features/F-049-dashboard-executivo-do-contrato/feature.md) — contrato em 2026-09-05; **unknown 1 respondido e prioridade definida no mesmo dia**: a tela é do **gestor do contrato na executora** (não do diretor nem do fiscal), `MEDIUM`. Sem domínio novo — o consolidado já guarda contratado, vigente, acumulado, saldo e a série por período. **Design Approval Package aprovado em 2026-09-05** (revisão 1) — todos os gates cumpridos, pronta para o Planner |
| F-050 | MEDIUM | READY_FOR_PLANNING | [Relatório de risco e pendências do período, pronto para o cliente](../features/F-050-relatorio-de-risco-do-periodo/feature.md) — contrato em 2026-09-05; **os dois unknowns respondidos**: documento do **período de medição** (sai junto do BM) e **testemunha divergente entra como observação**, nunca como risco de dinheiro (preserva o ADR-0049). `MEDIUM`, depois da F-049. **Design Approval Package aprovado em 2026-09-05** (revisão 1) — todos os gates cumpridos, pronta para o Planner |
