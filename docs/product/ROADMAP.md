...
| F-040 | HIGH | READY_FOR_HUMAN_REVIEW | [RE-RA declarada e a medição seguinte](../features/F-040-re-ra-e-medicao-seguinte/feature.md) — o aceite de 2026-08-28 fechou o mecanismo; a T6 e a T7 fecham os três desvios do pacote de design que a captura expôs, e a recaptura dos seis estados foi feita em 2026-09-01. Falta o merge |
...
| 19 | Próximo — medição além do v1 / Quantitativo automático derivado do scene graph aprovado; porta: `TakeoffItem.source` discriminado + `QuantitySource` lendo o `quantitativos.csv` do export DXF. Depende de identidade estruturada de elemento nas entidades (hoje rótulo é texto livre). | EM ESPECIFICAÇÃO | [Roadmap](ROADMAP.md), [ADR-0058](../adr/0058-quantitativo-derivado-do-scene-graph-e-identidade-de-elemento.md) | Registrado como [issue #102](https://github.com/biahflow/croquito/issues/102). O bloqueio nomeado — identidade estruturada de elemento — é o objeto do [ADR-0058](../adr/0058-quantitativo-derivado-do-scene-graph-e-identidade-de-elemento.md), que propõe `Entity.element_ref` atribuído por ato humano na revisão e a divergência cena×legenda como diagnóstico, nunca sobrescrita. O ADR está `Proposed` e **aguarda aceite humano** antes do planejamento. |
| 20 | Próximo — medição além do v1 / Criação e gestão de re-ratificações (RE-RA); no v1 elas são apenas lidas para o cálculo de saldo. | ENTREGUE, AGUARDANDO MERGE | [Roadmap](ROADMAP.md), [F-040](../features/F-040-re-ra-e-medicao-seguinte/feature.md), [ADR-0056](../adr/0056-re-ra-declarada-e-o-consolidado-da-medicao-seguinte.md) | O efeito da RE-RA sobre o saldo já está entregue e validado desde o M8 (`validate_amendments`); o que falta é procedência, caminho de criação e a **medição seguinte**, que não existe na `/v1` — sem ela a RE-RA só seria declarável no período 1. A [F-040](../features/F-040-re-ra-e-medicao-seguinte/feature.md) ataca os três, e o ADR-0056 aguarda aceite humano. A [F-040](../features/F-040-re-ra-e-medicao-seguinte/feature.md) entregou os três: o ADR-0056 foi aceito, o código está na `main` (PRs #109, #112 e #113) e a T6/T7 — a porta da medição seguinte, a herança, e a prévia calculada pelo servidor — está em revisão humana com os seis estados recapturados em 2026-09-01. |
