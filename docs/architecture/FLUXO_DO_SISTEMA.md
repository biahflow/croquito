...
| `WORKSITE_NAME_DOES_NOT_FIT_SHEET` | `valuation/template.py`, chamado na **abertura da rodada** (`POST /v1/valuation-rounds`, quando o nome viaja no corpo) e de novo por `valuation/worksite_calc.py` na **composição** do boletim | o nome da obra (com o sufixo da folha, na praça de N folhas) não cabe no nome de aba nem na forma curta. A recusa acontece primeiro onde o nome é **digitado**, e só depois onde o rótulo derivado nasce; na publicação do `.xlsx` a praça inteira já estaria montada, servida e aprovada, e o humano não teria mais o que fazer a não ser refazer |
