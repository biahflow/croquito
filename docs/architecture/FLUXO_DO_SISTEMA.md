...
| `SITE_SETUP_PARAMETER_MISSING` / `SITE_SETUP_CODE_ABSENT` | `valuation/site_setup.py`, na pré-visualização **e** na aplicação do acervo de canteiro | o acervo cita parâmetro de obra que a rodada não declarou (a recusa **nomeia todos**), ou código que o catálogo da cascata não tem (nomeia o código). Falha fechada: nenhuma parcela nasce parcialmente, e a recusa chega já na conferência — não depois de mexer na matriz |
