...
from croquitodxf_api.auth import OidcAuthenticator, Principal, require_principal
...
from croquitodxf_api.storage import ArtifactStore
...
from croquitodxf_worker.providers import ReviewChatOutput
...
# `croquitodxf_worker.proposal_calibration.revalidate_calibration`/`matrix_of`; the CLI's
...
def create_app(settings: ApiSettings | None = None, database: Database | None = None) -> FastAPI:
    ...
    async def problem_handler(request: Request, exception: StarletteHTTPException) -> JSONResponse:
        ...
        return JSONResponse(
            ...
            content={
                "type": f"https://errors.croquitodxf.local/{(code or 'http-error').lower()}",
    ...
    async def domain_error_handler(
        ...
        return JSONResponse(
            ...
            media_type="application/problem+json",
            content={
                "type": "https://errors.croquitodxf.local/domain-validation",
                "title": "Cena inválida",
                "status": 422,
