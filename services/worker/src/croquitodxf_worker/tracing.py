...
from croquitodxf_worker.dimension_annotation import NOTE_SOURCE_TYPE
...
from croquitodxf_worker.topology import JUNCTION_TOLERANCE_RATIO, Topology, build_topology
...
def _uuid(dataset_id: str, feature_id: str, suffix: str) -> UUID:
    return uuid5(NAMESPACE_URL, f"croquitodxf:{dataset_id}:{feature_id}:{suffix}")
...
def approve_trace(result: TraceSolveResult, approval: SceneApproval) -> ApprovedTraceRevision:
    ...
    approved_scene = declared_scene.model_copy(
        ...
        update={
            "id": uuid5(
                NAMESPACE_URL,
                f"croquitodxf:{result.scene.id}:{approval.approval_id}:approved",
            ),
