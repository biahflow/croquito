...
def main() -> int:
    parser = argparse.ArgumentParser(prog="croquitodxf-demo")
    ...
    if args.command == "seed-review":
        from croquitodxf_worker.criteria import ScopeCriterionError, parse_criterion_declaration
    ...
    if args.command == "refresh-proposals":
        from croquitodxf_worker.local_queue import LocalWorkerSettings
        from croquitodxf_worker.review_refresh import (
            RefreshInputs,
            RefreshRefusedError,
            refresh_proposals,
        )
    ...
    if args.command == "local-worker-once":
        from croquitodxf_worker.local_queue import run_local_worker_once
    ...
    if args.command == "propose-dataset":
        from croquitodxf_worker.vision_dataset import propose_dataset
    ...
    if args.command == "extraction-eval":
        ...
        from croquitodxf_worker.providers import build_extraction_arm
    ...
    if args.command == "register-extraction":
        from croquitodxf_worker.extraction_eval import register_extraction_arms
    ...
    if args.command == "transcribe-readings":
        from croquitodxf_worker.extraction_eval import ExtractionCandidate
        from croquitodxf_worker.providers import build_extraction_arm
        from croquitodxf_worker.transcription import run_transcription
    ...
    if args.command == "readings-to-packet":
        from croquitodxf_worker.review import load_review_packet
    ...
    if args.command == "review-artifacts":
        from croquitodxf_worker.review import load_review_packet, write_review_artifacts
    ...
    if args.command == "associate-review":
        from croquitodxf_worker.association import (
            associate_readings,
            load_proposal_set,
    ...
    if args.command == "rectangle-export":
        ...
        from croquitodxf_worker.review import SceneApproval
    ...
    if args.command == "solve-trace":
        ...
        from croquitodxf_worker.vision import VisionProposal, VisionProposalSet
    ...
    if args.command == "provider-contract-demo":
        from croquitodxf_worker.association import write_association_set
        from croquitodxf_worker.provider_review import build_provider_review_snapshot
        from croquitodxf_worker.providers import build_synthetic_provider_suite
        from croquitodxf_worker.review import write_review_artifacts
